...
//*********************************************************************************************
void validate(){
    int key; // integer value for shifting key
    string userChoice; // string value for user choice whether what they want to do
    string userMessage; // string value for message that will be changed
    //Actions to do if the user doesn't choose to exit
    while (userChoice != "exit"){
            cout << "What would you like to do to a message? (encrypt, decrypt, exit)" << endl;//Asks the user what they would like to do
            cout << "Enter your choice: ";
            cin>>userChoice;

            while(userChoice != "encrypt" && userChoice != "decrypt" && userChoice != "exit"){//Validating input for what the user chose to do
            ...
            if (userChoice == "exit"){ //Action to take if user chooses to exit
                    cout << endl;
                    cout << "Mesut Apil Security Systems"<<endl;
                    cout <<"2-25-2019"<<endl;
                    exit(0);
            }
...
void encrypting(int key, string userMessage){
    ...
    vector<vector<char> > shiftedAlphaNum(2,alphaNum);//Creating a 2d vector and making alphaNum as the 1st dimension of it
    ...
    for (int i = 1; i <= 1; i++){
        for (int j = 0; j < 36-key; j++){//Shifts the input of the vector according to the key
